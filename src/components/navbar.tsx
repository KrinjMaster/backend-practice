...
const Navbar = () => {
    ...
    COOKIEALL().then((response) => {
        setUrl(response[0].value)
    })

    return (
        <div className="w-[30vw] h-screen">
            <div className='flex flex-col items-center gap-3'>
                    <Link href={url === null ? '/users' : url} className={`${pathname === url ? 'text-white bg-indigo-600' : 'text-gray-500'} flex text-white text-2xl font-bold hover:text-white px-3 rounded-lg w-full items-center gap-3 transition-all duration-150 ease-in-out`}>
                    ...
                    <Link href={'/users'} className={`${pathname === '/users' ? 'text-white bg-indigo-600' : 'text-gray-500'} flex text-2xl font-bold hover:text-white px-3 rounded-lg w-full items-center gap-3 transition-all duration-150 ease-in-out`}>
                        <svg className="fill-current p-1 h-10 w-10" viewBox="0 0 18 18">
                    ...
                    <Link href={''} className={`${pathname === '/settings' ? 'text-white bg-indigo-600' : 'text-gray-500'} flex text-2xl font-bold hover:text-white px-3 rounded-lg w-full items-center gap-3 transition-all duration-150 ease-in-out`}>
                        <svg className="fill-current p-1 h-10 w-10" viewBox="0 0 18 18">
